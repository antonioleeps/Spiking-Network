function [steadypoint] = plotstatespace(w, plotting, tau) 
...
dE_1_E2 = -E_1_plus + Factivation((w*E_1_plus - E2 - 0.5));
idk = [];
...
for i = 1:length(idk)
    ...
    dE_1_E2_temp = -E_1_temp + Factivation(w*E_1_temp - E2_temp - 0.5);
    ...
    steadypoint = [steadypoint [E_1_plus_2;E2_temp_2]];
end

dE_1_E2 = -E_1_minus + Factivation(w*E_1_minus - E2 - 0.5);
idk = [];
for i = 1:length(dE_1_E2)-1
    if dE_1_E2(i)*dE_1_E2(i+1) <0
        idk = [idk i];
...
while plotting == 1
    ...
    F1 = (-XV + Factivation(w*XV - YV - 0.5))/tau;
    F2 = (-YV + Factivation(XV - YV - 0.5))/tau;
    ...
    scatter(steadypoint(1,:), steadypoint(2,:), 'o', 'DisplayName', 'E_{ss}');
